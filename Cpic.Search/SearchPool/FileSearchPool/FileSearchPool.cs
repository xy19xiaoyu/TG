...
*	文 件 名: FileSearchPool.cs
*	创 建 人: xiwenlei(xiwl) $ chenxiaoyu(xy)
*	创建日期: 2011-11-2 15:39:40
*	版    本: V1.0
*	备注描述: $Myparameter1$           
*
* 修改历史: 
...
using System.Linq;
using System.Text;
using Cpic.Cprs2010.Search.FileMeerySearch;
using Cpic.Cprs2010.Search;

/// <summary>
/// 为CPRS2010组件，提供对检索、检索历史及用户的相关操作类
/// </summary>
namespace Cpic.Cprs2010.SearchPool.FileSearchPool
{
    /// <summary>
    ///FileSearchPool 的摘要说明
    /// </summary>
    public class FileSearchPool : SearchPool.ISearchPool
    {
        ...
        public FileSearchPool(int min, int max, int growth, string ip, string port, int waitsecond, SearchDbType type)
        {
            IP = ip;
            Port = port;
            _Min = min;
        ...
        public void Ini()
        {
            ...
            if (!(string.IsNullOrEmpty(IP) || string.IsNullOrEmpty(Port)))
            {
                ...
                for (int i = 1; i <= Min; i++)
                {
                    FileMeerySearch ss = new FileMeerySearch(i, IP, Port, _type); ; //初始化
                    ss.logIn(); //登录Main
        ...
        public bool Grow()
        {
            ...
            if (Count < Max)
            {
                ...
                {
                    ISearch s = new FileMeerySearch(Count + 1, IP, Port, _type);
                    s.logIn();
                    lock (_FreeItems)
                    {
