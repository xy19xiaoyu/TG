...
function docdbTableSearch() {
    ...
    else if (res.length > nEnTbQueryMaxLen) {
        showError("检索式不能超过" + nEnTbQueryMaxLen + "个字符！");
        setSearchFormula("");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (msg) {
            ...
            if (strMsg.indexOf("<hit") != -1) // main返回正确结果
            {
                ...
                var viewPage = "docdb/docdbGeneralLists.html?";
                if (num == "0" || num == 0) {
                    alert("检索结果为零！");
                    return;
                } else {
...
function docdbGenerateSearch() {
    ...
    if (res.substring(0, 5) == "Error")
        ...
        obj.value = IsOldCCSearch ? res : getMergeSearchEndFlag(res); // obj.value = res;
...
function getTableSearchQuery() {
    ...
    if (res.length != 0) {
        ...
        if (IsOldCCSearch) {
            if (aN && pN) {
                var coutrys = GetCountrys();
                var coutry = "(";
                if (coutrys.length > 6) {    //(coutrys[coutrys.length - 2] == "ELSE") {
                    coutrys = GetNotCountrys();
                    for (i = 0; i < coutrys.length - 1; i++) {
                        coutry += coutrys[i] + "/AN+";
                    }
                    ...
                    if (coutrys.length == 2) {
                        if (/\w{0,14}\/\s*(AN)/g.test(res) || /\w{0,14}\/\s*(PN)/g.test(res)) {
                            var reg = /\w{0,14}\/\s*(AN)/g;
                            if (reg.test(res)) {
                                var str = res.match(reg);
                                res = res.replace(reg, coutrys[0] + str[0]);
                            }
                            reg = /\w{0,14}\/\s*(PN)/g;
                            if (reg.test(res)) {
                                var str = res.match(reg);
...
// var m_Entrances = "AN,AD,PN,PD,IC,PR,IN,PA,TI,AB,CT,EC,";
