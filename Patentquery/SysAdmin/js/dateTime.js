...
document.write("<iframe name='meizzCalendarIframe' scrolling='no' frameborder='0' width='150' height='220'></iframe></div>");

function writeIframe() {
    var strIframe = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=utf-8'><style>" +
    "*{font-size: 12px; font-family: 宋体}" +
    ".bg{  color: " + WebCalendar.lightColor + "; cursor: default; background-color: #6B9AC6;}" +
    ...
    "case 37 : parent.prevM(); break; case 38 : parent.prevY(); break; case 38 : parent.nextM(); break; case 40 : parent.nextY(); break;" +
    ...
    strIframe += "<tr>";
    for (var x = 35; x < 38; x++) strIframe += "<td class=out id='meizzDay" + x + "'></td>";

    //新加清楚日期功能
    strIframe += "<td  colspan=2 class=out title='" + WebCalendar.ClearDate + "'><input style=' background-color: " +
         WebCalendar.btnBgColor + ";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'" +
         " type=button value='清除' onclick='parent.ClearDate()'></td>"

    strIframe += "<td colspan=2 class=out title='" + WebCalendar.regInfo + "'><input style=' background-color: " +
         WebCalendar.btnBgColor + ";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'" +
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.regInfo += "";
    //this.ClearDate = "清楚日期的快捷键：[Del]";
    this.ClearDate = "清除日期";
    this.daysMonth = new Array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
    this.day = new Array(38);            //定义日历展示用的数组
    this.dayObj = new Array(38);            //定义日期展示控件数组
    ...
    this.yearFall = 200;           //定义年下拉框的年差值

    this.format = "yyyy-mm-dd"; //回传日期的格式

    this.timeShow = false;        //是否返回时间
    this.drag = true;         //是否允许拖动
    this.darkColor = "0000D0"; 	//"#B1CAF5";	//"#D70D03"   //控件的暗色

    this.lightColor = "#FFFFFF"; //"#FFFFFF";    //控件的亮色

    this.btnBgColor = "#E6E6FA"; //"#FDDADC";    //控件的按钮背景色
    this.wordColor = "#000080"; //"#000080";    //控件的文字颜色

    this.wordDark = "#DCDCDC"; //"#DCDCDC";    //控件的暗文字颜色
    this.dayBgColor = "#F5F5FA"; //"#F5F5FA";    //日期数字背景色

    this.todayColor = "#FF0000"; //"#FFB5B7";    //今天在日历上的标示背景色
    this.DarkBorder = "#D4D0C8"; //"#FDDADC";    //日期显示的立体表达色
}
...
function funYearSelect() //年份的下拉框
{
    ...
    var min = (y - n >= 1900) ? y - n : 1900;
    var max = (y + n <= 2020) ? y + n : 2020;
