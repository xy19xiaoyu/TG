...
function cnTableSearch(lang) {
    ...
    var view = '查看';
    var viewPage = "../cn/cnShwGeneral.aspx?";
    if (lang == 'En') {
        view = 'View';
        viewPage = "../cn/cnShwGeneral.aspx?Ty=E&";
    }
        ...
        success: function (msg) {
            ...
            if (strMsg.indexOf("<hit") != -1) // main返回正确结果
            {
                ...
                if (num == "0" || num == 0) {
                    ...
                    addToTab(viewPage + "No=999&Query=" + encodeURIComponent(res) + "&Nm=" + num, 'CN000', true);
                    return;
                }

                var strView = "";
                if (parseInt(num) != 0)
                    strView = "&nbsp;&nbsp;<a href='#' onclick=\"addToTab('" + viewPage + "No=999&Query=" + encodeURIComponent(res) + "&Nm=" + num + "','CN000',true)\" >" + view + "</a>";
...
// var res = "F XX (200302/AD + 200201/AD) * (计算机/TI)";
// var res = "F XX ( (200302/AD + 200201/AD) * (计算机/TI)) + (200501/AD)";
var m_EntrancesCn = "AN,PN,IC,PR,IN,PA,AG,TI,CL,AD,PD,CT,CO,DZ,TX,AB,DS,CS,GN,GD,AE,IE,PE,TE,EC,MC,AT,"; //所有入口,以，连接多个
...
function validateBib(objTxt) {
    ...
    var res = validateLogicSearchQueryCn(txtId, strVal);
    if (res.indexOf("Error:") != -1) {
        setViewResult(res);
        return;
    }

    // setSearchFormula(getTableSearchQueryCn());
}
